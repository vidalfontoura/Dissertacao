...
Este capítulo apresenta o referencial teórico das estratégias heurísticas implementadas para resolver o problema PDP nesta Dissertação. Inicialmente, os algoritmos evolutivos e otimização multi objetiva são apresentados. Em seguida os algoritmos multi objetivos NSGAII e IBEA são descritos em detalhe. As hiper heurísticas são apresentadas em sequência pois estas abordagens foram exploradas durante o período de desenvolvimento dessa Dissertação. Finalmente programação genética e evolução gramatical são apresentadas pois estes conceitos foram utilizados na Metodologia deste trabalho.
